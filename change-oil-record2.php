...
$query = "
    ...
    WHERE t.wheel_name IN (1, 4, 5, 6, 7)  -- Add this WHERE clause to filter
    ORDER BY t.trck_id, co.column2
";
...
foreach ($results as $row) {
    ...
    if ($row['column2'] !== null && $row['column2'] != 0) {
        $trucks[$trck_id]['readings'][$row['column2']] = [
...
$column2Query = "SELECT DISTINCT `column2` FROM change_oil ORDER BY `column2` ASC";
$column2Stmt = $pdo->prepare($column2Query);
$column2Stmt->execute();
$columns2 = $column2Stmt->fetchAll(PDO::FETCH_COLUMN);
...
$display_query = "
    ...
    JOIN type wt ON t.wheel_name = wt.w_id
    LEFT JOIN change_oil co ON t.trck_id = co.trck_id
    WHERE t.wheel_name IN (1, 4, 5, 6, 7)  -- Filter only for display
    ORDER BY t.trck_id, co.column2
";
...
<aside id="sidebar" class="sidebar">
    ...
    <ul class="sidebar-nav" id="sidebar-nav">
        ...
        <li class="nav-item">
        <a class="nav-link" href="change-oil-record2.php">
        ...
        <h1> Truck Changed Oil Record</h1>
            <nav>
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active">10W HINO | 6W FUSO | 6W ACU | 6W EV FOTON | 4W FUSO</li>
                </ol>
            </nav>
        </div>
                    ...
                    <div class="row">
                        <div class="container mt-4">
                            <div class="controls-container">
                                ...
                                <button class="btn btn-warning mb-3" onclick="editColumn2()">Edit Column</button>
                                ...
                                <div class="search-filter-wrapper">
                                    ...
                                    <div class="column2-filter">
                                        <select class="form-select" id="ColumnFilter">
                                            <option value="all">All Columns</option>
                                            <option value="1-10">Column2 1-10</option>
                                            <option value="11-20">Column2 11-20</option>
                                        </select>
                                    </div>
                                </div>
                            </div>
                                            ...
                                            <thead>
                                                <tr>
                                                    ...
                                                    <?php foreach ($columns2 as $column2): ?>
                                                        <?php if (!is_null($column2) && $column2 !== ''): ?> 
                                                            <th style="width: 200px;">
                                                                <?php echo htmlspecialchars(number_format($column2)); ?>
                                                            ...
                                                            <?php if (!is_null($column2) && $column2 !== ''): ?> 
                                                                <td class="clickable-cell" 
                                                                    onclick="openModal('<?php echo htmlspecialchars($truck['trck_id']); ?>', 
                                                                                    '<?php echo htmlspecialchars($column2); ?>', 
                                                                                    '<?php echo htmlspecialchars($truck['trck_plate']); ?>', 
                                                                                    '<?php echo htmlspecialchars($truck['wheel_type']); ?>')"
                                                                    data-trck-id="<?php echo htmlspecialchars($truck['trck_id']); ?>"
                                                                    data-column2="<?php echo htmlspecialchars($column2); ?>">

                                                                    <?php 
                                                                    $reading = $truck['readings'][$column2] ?? null;
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        <label class="form-label">Column2 Number</label>
                        <input type="text" class="form-control" id="column2" readonly>
                    </div>
                    <div class="mb-3">
...
<script>
    ...
    document.getElementById('ColumnFilter').addEventListener('change', function() {
        ...
        headerCells.forEach((cell, index) => {
            if (index >= columnStartIndex) {
                ...
                const columnNumber = parseInt(columnText.replace('Column2 ', ''));
        ...
        dataCells.forEach(row => {
            ...
            cells.forEach((cell, index) => {
                if (index >= columnStartIndex) {const columnNumber = parseInt(headerCells[index].textContent.replace('Column2 ', ''));
                    if (selectedRange === 'all') {cell.style.display = '';
                    } else if (selectedRange === '1-10') {cell.style.display = (columnNumber >= 1 && columnNumber <= 10) ? '' : 'none';
                    } else if (selectedRange === '11-20') {cell.style.display = (columnNumber >= 11 && columnNumber <= 20) ? '' : 'none';
                    }
    ...
    function openModal(trck_id, column2, trck_plate, wheel_type) {
        ...
        document.getElementById('saveOdometerBtn').dataset.columnNo = column2;
    ...
    function addColumn() {
        var newColumn2 = prompt("Enter a new column2 number:");
        if (!newColumn2) {alert("Please enter a column2 number.");return;}

        newColumn2 = parseInt(newColumn2);
        if (isNaN(newColumn2) || newColumn2 <= 0) {alert("Invalid column2 number. Please enter a valid number.");return;}

        fetch('add_column2.php', {method: 'POST',headers: {'Content-Type': 'application/json',},body: JSON.stringify({column2: newColumn2})})
        .then(response => response.json())
        .then(data => {
            if (data.success) {alert("column2 " + newColumn2 + " added successfully!");location.reload();} 
            else {alert('Error adding column2: ' + (data.message || 'Unknown error'));}})
        .catch(error => {console.error('Error:', error);alert('Failed to connect to server.');});
    }

    function editColumn2() {
        var oldColumn2 = prompt("Enter the existing column2 number to edit:");
        var newColumn2 = prompt("Enter the new column2 number:");

        if (!oldColumn2 || !newColumn2) {
            alert("Please enter both old and new column2 numbers.");
            return;
        }

        oldColumn2 = parseInt(oldColumn2);
        newColumn2 = parseInt(newColumn2);

        if (isNaN(oldColumn2) || isNaN(newColumn2) || newColumn2 <= 0) {
            alert("Invalid column2 number. Please enter valid numbers.");
            return;
        }

        fetch('edit_column2.php', {
            method: 'POST',
            headers: {'Content-Type': 'application/json'},
            body: JSON.stringify({old_column2: oldColumn2, new_column2: newColumn2})
        })
        .then(response => response.json())
        .then(data => {
            if (data.success) {
                alert("column2 updated successfully!");
                location.reload();
            } else {
                alert("Error updating column2: " + (data.message || 'Unknown error'));
            }
        })
        .catch(error => {
    ...
    document.getElementById('saveOdometerBtn').addEventListener('click', function() {
        ...
        let column2 = this.dataset.columnNo;
        ...
        fetch('update_change_oil_c2.php', {method: 'POST',headers: { 'Content-Type': 'application/json' },body: JSON.stringify({ trck_id, column2, change_oil_odo, date })})
        .then(response => response.json())
        .then(data => {
            if (data.success) {let cell = document.querySelector(`td[data-trck-id="${trck_id}"][data-column2="${column2}"]`);
